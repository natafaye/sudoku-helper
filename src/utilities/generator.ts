 ...
 */
export const generatePossibilities = (
  sum: number,
  numSquares: number,
  ...
  if (
    ...
    sum <= 0 ||
    sum > 45 ||
    includedDigits.some((d) => excludedDigits.includes(d))
  )
    return [];
  ...
  if (includedDigits.length) {
    ...
    if (numSquares === 0 || sum === 0)
      return sum === 0 && numSquares === 0 ? [[...includedDigits]] : [];
  }

  // Handle if there's only one square to fill
  if (numSquares === 1)
    return sum <= 9 && !excludedDigits.includes(sum)
      ? [[sum, ...includedDigits]]
      : [];
  ...
  for (let currDigit = 1; currDigit < sum / numSquares; currDigit++) {
    ...
    const newPossiblities = remainderPossiblities.map((digits) => [
      currDigit,
      ...digits,
    ]);
